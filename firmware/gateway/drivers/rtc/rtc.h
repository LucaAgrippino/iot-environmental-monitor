/**
 * @file rtc.h
 * @brief CMSIS-level RTC driver — wall-clock time and backup-register access (Gateway).
 *
 ...
 * synchronised.
 *
 * @note Realised on the backup-domain RTC of the STM32L475 (Gateway). LSE
 *       32.768 kHz is the clock source. The Field Device counterpart
 *       (firmware/field-device/drivers/rtc/rtc_driver.h) implements the
 *       same API for the STM32F469 — separate CubeIDE projects, separate
 *       files (same split as DebugUartDriver), even though the register
 *       layout is identical (rtc-driver.md §4.4).
 ...
 * power-off on this board (VBAT tied to VDD).
 *
 * DBP is set by rtc_init() and remains set; callers do not need to manage it.
 *
 * @param idx     Backup register index. Valid range: 0..31. Out-of-range
 *                returns RTC_ERR_BACKUP_BOUNDS.
 ...
 *
 * @param idx   Backup register index. Range: 0..31.
