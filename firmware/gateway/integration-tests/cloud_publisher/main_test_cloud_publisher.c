 ...
 *   5. Fill in BRINGUP_WIFI_SSID / BRINGUP_WIFI_PASSWORD and
 *      BRINGUP_MQTT_* below — identical setup to MqttClient's own
 *      bring-up (see that file's header comment for the local Mosquitto
 *      broker option).
 *
 ...
 *   TC-HW-CP-006  health timer fires; a JSON health frame is observed
 *                 publishing on dt/iotmonitor/<serial>/health
 *   TC-HW-CP-007  stats timer (1 Hz) drives mqtt_client_get_stats() +
...
void alarm_service_subscribe(alarm_service_handle_t handle, alarm_event_cb_t cb, void *ctx)
{
    ...
    (void) cb;
    (void) ctx;
    LOG_INFO("CloudPub", "TC-HW-CP-004  alarm_service_subscribe() called (stand-in, never fires)");
}
                               ...
                               const mqtt_stats_t *last_stats)
{
    (void) handle;
    (void) last_stats;
    LOG_INFO("CloudPub", "TC-HW-CP-007  stats poll: publishes_sent=%lu publish_failures=%lu",
...
static void cloud_publisher_bringup_task(void *arg)
{
    ...
    mqtt_client_config_t mqtt_config = {
        .wifi = wifi_handle,
        .msg_cb = NULL,        /* CP-O5: not yet wired to CloudPublisher */
        .disconnect_cb = NULL, /* CP-O5: not yet wired to CloudPublisher */
    };
    ...
    {
        bringup_fail("TC-HW-CP-004  cloud_publisher_create() failed");
    }
