...
/* Slot A CRC field physical QSPI address (slot A base 0x00000000 + 0x7FF8) */
#define CS_SLOT_A_CRC_ADDR (0x00007FF8UL)
...
static void test_task(void *param)
{
    ...
    check("flash erase", config_store_erase(), CONFIG_STORE_OK);
    ...
    check("save blob_a (8 bytes)", config_store_save(k_blob_a, sizeof(k_blob_a)), CONFIG_STORE_OK);

    /* 3 — check_integrity after one valid slot */
    check("check_integrity after save", config_store_check_integrity(), CONFIG_STORE_OK);

    /* 4 — Load blob_a and verify contents */
    len = 0U;
    check("load blob_a", config_store_load(buf, &len, sizeof(buf)), CONFIG_STORE_OK);
    ...
    check("save blob_b (16 bytes)", config_store_save(k_blob_b, sizeof(k_blob_b)), CONFIG_STORE_OK);

    /* 6 — Load blob_b; slot A (seq=2) should be selected */
    len = 0U;
    check("load blob_b", config_store_load(buf, &len, sizeof(buf)), CONFIG_STORE_OK);
     ...
     *     Slot A CRC field is at physical address 0x00007FF8. */
    ...
    len = 0U;
    check("load after slot A corrupt", config_store_load(buf, &len, sizeof(buf)), CONFIG_STORE_OK);
    if (len == sizeof(k_blob_b) && (memcmp(buf, k_blob_b, sizeof(k_blob_b)) == 0))
    {
    ...
    check("config_store_erase()", config_store_erase(), CONFIG_STORE_OK);
...
int main(void)
{
    ...
    if (config_store_init((ihealth_report_t *) &s_health_stub) != CONFIG_STORE_OK)
    {
