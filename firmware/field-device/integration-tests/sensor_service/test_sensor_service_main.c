 ...
 * 2 | "[AS] AlarmService initialised" printed at startup    | alarm_service_init() succeeds | | 3 |
 * "[IT] Cycle <N> T=xx.xx H=xx.xx P=xxxx.x" at ~5 Hz   | Pipeline running (bug: 5Hz not 10Hz)   |
 * | 4 | Readings change slowly (IIR filter visible)           | IIR filter applied | | 5 | "[IT]
 * ALARM RAISED HIGH TEMPERATURE" when T > 35°C   | AlarmService threshold detection       | | 6 |
 * "[IT] ALARM CLEARED TEMPERATURE" after T drops < 33°C | Hysteresis working correctly           |
 * | 7 | "is_ready: 1" appears after first clean cycle         | sensor_service_is_ready() returns
 * true |
 *
...
static void alarm_event_cb(sensor_id_t sensor, alarm_event_t event, const sensor_reading_t *reading)
{
...
int main(void)
{
    ...
    (void) xTaskCreateStatic(vSensorTask, "SensorTask", SENSOR_TASK_STACK_WORDS, NULL,
                             SENSOR_TASK_PRIORITY, s_sensor_task_stack, &s_sensor_task_tcb);
