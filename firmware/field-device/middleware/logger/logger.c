...
LOGGER_TEST_VISIBLE uint16_t format_line(const log_entry_t *e,
                                         ...
                                         char *out, size_t out_size)
{
    ...
    static const char *const level_color[] = {
        [LOG_LEVEL_ERROR] = ANSI_BRIGTH_RED,
    ...
    n = snprintf(out, out_size, "%s[%s]%s",
                 level_color[lvl], level_tag[lvl], ANSI_RESET);
#else
    ...
    if (e->use_uptime)
    {
        n = snprintf(out + written, out_size - (size_t)written,
                     "[T+%08lu]", (unsigned long)e->ts.uptime_ms);
    }
    else
    {
        n = snprintf(out + written, out_size - (size_t)written,
                     "[%02u:%02u:%02u]",
                     (unsigned)e->ts.wallclock.hour,
                     (unsigned)e->ts.wallclock.minute,
...
logger_err_t logger_init(log_level_t initial_level)
{
    ...
    s_queue = xQueueCreateStatic(LOGGER_QUEUE_DEPTH,
                                 sizeof(log_entry_t),
                                 s_queue_storage,
                                 &s_queue_ctrl);

    s_drain_task = xTaskCreateStatic(drain_task_body,
                                     "log_drain",
                                     LOGGER_DRAIN_STACK_WORDS,
                                     NULL,
...
void logger_log(log_level_t level, const char *module, const char *msg)
{
    ...
    if (!level_in_range(level))     { return; }
    if (level > s_log.level)        { return; }    /* runtime filter */
    if (msg == NULL)                { return; }
