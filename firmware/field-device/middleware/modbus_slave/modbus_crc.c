/**
 * @file modbus_crc.c
 * @brief CRC-16/IBM (Modbus) implementation — 256-entry lookup table.
 *
 * Table generated from reflected polynomial 0xA001 (bit-reverse of 0x8005).
 * 256 × sizeof(uint16_t) = 512 bytes of flash.
 *
 * Verified against the known Modbus RTU test vector:
 *   frame  = {0x01, 0x03, 0x00, 0x00, 0x00, 0x02}
 *   result = 0x0BC4  (appended: 0xC4, 0x0B)
 */

#include "modbus_crc.h"

/* CRC-16/IBM lookup table (polynomial 0xA001, reflected 0x8005). */
...
uint16_t modbus_crc16(const uint8_t *buf, uint16_t len)
{
    ...
    while (len--)
    {
        crc = (crc >> 8U) ^ s_crc_table[(crc ^ (uint16_t) *buf++) & 0x00FFU];
    }
    return crc;
}
