...
static qspi_flash_err_t cs_flash_erase_range(uint32_t base, uint32_t sectors)
{
    ...
    for (s = 0U; s < sectors; s++)
    {
        qspi_flash_err_t err =
            qspi_flash_driver->erase_sector(base + (s * CONFIG_STORE_SECTOR_SIZE));
...
static qspi_flash_err_t cs_flash_write_bytes(uint32_t addr, const uint8_t *buf, uint32_t len)
{
    while (len > 0U)
    {
        uint32_t page_offset = addr & 0xFFU;
        uint32_t space = 256U - page_offset;
        uint32_t chunk = (len < space) ? len : space;
        qspi_flash_err_t err = qspi_flash_driver->write_page(addr, buf, (uint16_t) chunk);
        if (err != QSPI_FLASH_OK)
        {
            return err;
        }
...
{
    return (slot_index == 0U) ? CONFIG_STORE_QSPI_BASE_ADDR
...
static uint32_t cs_crc_feed_ff_pad(uint32_t crc, uint32_t n)
{
    while (n > 0U)
    {
        uint32_t chunk = (n > (uint32_t) sizeof(s_ff_pad)) ? (uint32_t) sizeof(s_ff_pad) : n;
...
CONFIG_STORE_TEST_VISIBLE bool cs_validate_slot(uint8_t slot_index, uint32_t *seq_out)
{
    ...
    /* Read stored CRC */
    if (cs_flash_read_bytes(base + CS_CRC_OFFSET, (uint8_t *) &stored_crc, sizeof(stored_crc)) !=
        QSPI_FLASH_OK)
    {
    ...
    {
        uint32_t rlen = CS_CRC_COVERAGE - off;
...
static void cs_select_target(const bool *valid, const uint32_t *seq, uint8_t *target_out,
...
static config_store_err_t cs_save_locked(const void *data, uint32_t len)
{
    ...
    if (cs_flash_write_bytes(target_base + CS_HEADER_SIZE, (const uint8_t *) data, len) !=
        QSPI_FLASH_OK)
    {
        cs_push_health(HEALTH_EVENT_CONFIG_WRITE_FAIL);
        return CONFIG_STORE_ERR_FLASH_WRITE;
    ...
    if (cs_flash_write_bytes(target_base + CS_CRC_OFFSET, (const uint8_t *) &crc_val,
    ...
    LOG_INFO(CS_MODULE_NAME, "saved %u bytes to slot %c (seq=%u)", (unsigned) len,
             (target_idx == 0U) ? 'A' : 'B', (unsigned) next_seq);
...
config_store_err_t config_store_init(const ihealth_report_t *health)
{
    ...
    if (cs_flash_read_bytes(CONFIG_STORE_QSPI_BASE_ADDR, (uint8_t *) &probe, sizeof(probe)) !=
        QSPI_FLASH_OK)
    {
...
config_store_err_t config_store_load(void *data_out, uint32_t *len_out, uint32_t max_len)
{
    ...
    if (cs_flash_read_bytes(base + CS_HEADER_SIZE, (uint8_t *) data_out, hdr.data_len) !=
        QSPI_FLASH_OK)
    {
        cs_push_health(HEALTH_EVENT_CONFIG_READ_FAIL);
        (void) xSemaphoreGive(s_cs.mutex);
