 ...
 * @brief Narrow stub for SensorService in alarm_service unit tests.
 *
 * Provides the data types and the sensor_service_subscribe() declaration
 * that alarm_service.c depends on. Including this header (via #ifdef TEST in
 * alarm_service.c) instead of sensor_service.h prevents Ceedling from
 * auto-linking sensor_service.c and its entire driver chain.
 *
