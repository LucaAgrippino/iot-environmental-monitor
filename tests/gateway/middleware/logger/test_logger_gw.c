/**
 * @file test_logger_gw.c
 * @brief Unit tests for Logger Middleware — Gateway (host build, Unity).
 *
 * Covers TC-LOG-001..022 per docs/lld/middleware/logger.md §7. Identical
 * test logic to the Field Device suite (tests/field-device/middleware/
 * logger/test_logger.c) — Logger's public API and behaviour are board-
 * agnostic (companion §1: Board = Both). Named test_logger_gw.c (not
 * test_logger.c) because Field Device already owns the Ceedling test
 * target :test_logger:; the production files stay bare (logger.c) on
 * both boards, per the board-suffix convention (test files only).
 *
 * Mocking strategy:
 ...
 * This is intentionally NOT rtc/rtc.h / debug_uart/debug_uart.h — including
 * those would make Ceedling auto-link the real driver .c files and collide
 * with the inline stubs. Shared with the Field Device Logger tests — the
 * types and signatures are identical on both boards. */
...
/* The other rtc functions are no longer referenced — driver_stubs.h
 * doesn't declare them, and the real rtc.c is not linked into this
 ...
 * above the rtc stub. Add stubs here only if logger.c ever calls more
...
static void stub_rtc_set_ok(uint8_t h, uint8_t m, uint8_t s)
{
    ...
    g_stub_rtc_get_time_value.month = 7U;
    g_stub_rtc_get_time_value.day = 6U;
...
{
    logger_err_t err = logger_init(LOG_LEVEL_INFO);
...
{
    logger_err_t err = logger_init((log_level_t) 99);
...
{
    logger_err_t err = logger_set_level(LOG_LEVEL_WARN);
...
void test_TC_LOG_010_set_level_out_of_range_returns_invalid_arg(void)
{
    ...
    logger_err_t err = logger_set_level((log_level_t) 42);
    TEST_ASSERT_EQUAL(LOGGER_ERR_INVALID_ARG, err);
}
...
void test_TC_LOG_021_error_line_starts_with_red_ansi(void)
{
    ...
    TEST_ASSERT_EQUAL_STRING_LEN("\033[91m[ERROR]\033[0m", (const char *) g_stub_uart_buf, 16);
}
...
void test_TC_LOG_022_other_levels_use_correct_ansi(void)
{
    ...
    logger_log(LOG_LEVEL_WARN, "M", "x");
    TEST_ASSERT_EQUAL_STRING_LEN("\033[93m[ WARN]\033[0m", (const char *) g_stub_uart_buf, 16);

    (void) memset(g_stub_uart_buf, 0, sizeof(g_stub_uart_buf));
    logger_log(LOG_LEVEL_INFO, "M", "x");
    TEST_ASSERT_EQUAL_STRING_LEN("\033[96m[ INFO]\033[0m", (const char *) g_stub_uart_buf, 16);

    (void) memset(g_stub_uart_buf, 0, sizeof(g_stub_uart_buf));
    logger_log(LOG_LEVEL_DEBUG, "M", "x");
    TEST_ASSERT_EQUAL_STRING_LEN("\033[97m[DEBUG]\033[0m", (const char *) g_stub_uart_buf, 15);
}
