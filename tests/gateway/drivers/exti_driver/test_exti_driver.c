/**
 * @file test_exti_driver.c
 * @brief Unit tests for ExtiDriver — Gateway (STM32L475).
 *
 * Covers EXTI-T01..T17 per docs/lld/drivers/exti-driver.md §7.
 *
 * ExtiDriver is Gateway-only for now (firmware/gateway/drivers/exti/); the
 * register-name alias block in exti_driver.c still resolves at compile time
 * from STM32L475xx / STM32F469xx so it can be reused as a shared module
 * later without changes, but only the L475 path is exercised here.
 *
 * Build: STM32L475xx and TEST must be defined (project.yml :test_exti_driver:).
 */
...
void test_EXTI_T01_configure_line1_port_e_rising(void)
{
    ...
    TEST_ASSERT_BITS(1u << 1u, 1u << 1u, EXTI->RTSR1);
    TEST_ASSERT_BITS(1u << 1u, 0u, EXTI->FTSR1);
}
...
void test_EXTI_T02_configure_line8_port_c_rising(void)
{
    ...
    TEST_ASSERT_BITS(1u << 8u, 1u << 8u, EXTI->RTSR1);
}
...
void test_EXTI_T03_configure_line11_port_c_rising(void)
{
    ...
    TEST_ASSERT_BITS(1u << 11u, 1u << 11u, EXTI->RTSR1);
}
...
void test_EXTI_T06_configure_line5_falling_edge(void)
{
    ...
    TEST_ASSERT_EQUAL(EXTI_ERR_OK, err);
    TEST_ASSERT_BITS(1u << 5u, 1u << 5u, EXTI->FTSR1);
    TEST_ASSERT_BITS(1u << 5u, 0u, EXTI->RTSR1);
}
...
void test_EXTI_T07_configure_line3_both_edges(void)
{
    ...
    TEST_ASSERT_EQUAL(EXTI_ERR_OK, err);
    TEST_ASSERT_BITS(1u << 3u, 1u << 3u, EXTI->RTSR1);
    TEST_ASSERT_BITS(1u << 3u, 1u << 3u, EXTI->FTSR1);
}
...
void test_EXTI_T08_enable_after_configure(void)
{
    ...
    TEST_ASSERT_EQUAL(EXTI_ERR_OK, err);
    TEST_ASSERT_BITS(1u << 1u, 1u << 1u, EXTI->IMR1);
...
void test_EXTI_T10_disable_after_configure_and_enable(void)
{
    ...
    TEST_ASSERT_EQUAL(EXTI_ERR_OK, err);
    TEST_ASSERT_BITS(1u << 1u, 0u, EXTI->IMR1);
...
void test_EXTI_T12_clear_pending_line8(void)
{
    ...
    TEST_ASSERT_EQUAL_HEX32(1u << 8u, EXTI->PR1);
}
