...
- description: A maximum or minimum value in which a sensor reading should be 
- attributes:
  - max_value: Magnitude
  - min_value: Magnitude
...
- ### SensorReading — Alarm
  - SensorReading --- Alarm                       not related     (denormalized into Alarm attrs)
- ### AlarmThreshold — Alarm
  - AlarmThreshold --- Alarm                      not related     (denormalized into Alarm attrs)
- ### Device — Alarm
...
Generalise Sensor into ScalarSensor and VectorSensor, with corresponding reading subtypes.
Model a single SensorReading composed of one or more MeasurementValue instances, where scalar readings contain one value and vector readings contain three with axis labels.
...
Generalise the three payload types into a common CloudPayload supertype.
Inline all three payload shapes into BufferedRecord with nullable attribute groups.
Model three separate composition relationships from BufferedRecord, each with multiplicity 0..1, plus an invariant constraining exactly one to be non-null.
...
Several entities carry identifier attributes that reference other entities: SensorReading.reading_id is its own identifier, while Alarm.sensor_id, Telemetry.device_serial_number, and similar are references to other entities. These appear alongside formal relationships on the diagram.
