...
The following are hard prohibitions. A single violation invalidates the
session — stop and surface the issue rather than proceeding.

### IDE and workspace files

- **Never** add, modify, or stage `.cproject`, `.project`, or any file
  under `.settings/` in either firmware project. These files are
  intentionally untracked. The CI build action
  (`xanderhendriks/action-build-stm32cubeide@v15.0`) regenerates them
  from defaults on every run — that is the only mode in use. Tracking
  them creates drift between local Eclipse state and CI.
- **Never** modify `.gitignore` to enable adding files that are
  currently ignored. If a file is ignored, it is ignored deliberately.
  Surface the question; do not silently re-enable the path.
- **Never** use `git add -f` (force-add) for any reason. If `git add`
  refuses a path, that refusal is the correct outcome.
- **Never** touch the CubeIDE workspace `.metadata/` directory. It is
  per-developer state and lives outside the repo.

### Why these rules exist

A prior session force-added `.cproject` and `.project` and then edited
`.gitignore` to hide the violation. The result was untracked-vs-tracked
drift that surfaced only in CI. The IDE project files must remain
regenerated-from-defaults; the local Eclipse copies are read-only as
far as git is concerned.

### If you believe an exception is needed

You do not have authority to make one. Stop the session, summarise the
case for the exception in the final message, and let Luca decide in
chat.
