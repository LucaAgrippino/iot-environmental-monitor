...
| SD-05 | Alarm propagation                     | one diagram (sunny + rainy via `alt` at publish point)             | FD ↔ GW ↔ Cloud           | `AlarmService` (FD, GW)              | UC-08, UC-09  | AM, NF-101/-113/-207          |
| SD-06 | OTA firmware update                   | SD-06a, SD-06b, SD-06c, SD-06d                                     | Cloud → GW                | `UpdateService`                      | UC-18, UC-20  | DM, NF-204                    |
| SD-07 | Remote configuration command          | one diagram (with profile-update branch)                           | Cloud → GW                | `ConfigService`                      | UC-15         | DM, NF-301                    |
| SD-08 | Remote restart                        | one diagram (with confirmation, restart, post-boot self-check)     | Cloud → GW                | `LifecycleController`                | UC-17         | DM, NF-203                    |
...
| —   |                         |                          |        |                                             | **ref** SD-04b (cloud connection on boot) |                              | CC-050                      |
...
| —   |                         |                          |        |                                             | **end alt**                               |                              |                             |
| —   |                         |                          |        |                                             | **end loop**                              |                              |                             |
| 15  | `LifecycleController`   | `Logger`                 | async  | log boot complete (link summary)            |                                           |                              |                             |
| 16  | `LifecycleController`   | `LifecycleController`    | self   | transition Init → Running                   |                                           |                              | (state-machines.md §7.2)    |

**UML note 1 (Option C — fall-through):** *"The gateway transitions to Running regardless of per-slave outcomes. Slaves marked `link_rejected` or `link_unlinked` are not polled; this is the industry-standard 'deny by default' behaviour. The cloud is informed of the per-slave link state via health telemetry (SD-03b)."*
...
| SRS         | Text fragment                                                                      | Label                                       |
|-------------|------------------------------------------------------------------------------------|---------------------------------------------|
| REQ-DM-073  | Retain previous firmware until self-check passes                                   | NO SEQUENCE IMPLICATION (architectural)     |
...
| —  |                       |                     |        |                                             | **end alt**                            |                                   |                                        |
| 6  | `LifecycleController` | `LifecycleController`| self  | transition Init → Running (or stay in SelfChecking until update outcome) |          |                                   | (state-machines.md §7.2)               |
...
| SD00-D2 | Per-slave probe with profile-bound device-ID validation; fall-through to Running on failure (Option C) | Industry-standard "deny by default"; supports REQ-NF-203 5 s budget without blocking on unreachable slaves         |
...
| 10  | `SensorService`        | `SensorService`      | self   | publish to ISensorReadings |                           |                         | SA-150         |
...
| 7 | `ModbusRegisterMap`            | «application» | FD    | Reads input registers from cached sensor data                  |
...
| 6   |     `ModbusSlave`           | `ModbusRegisterMap`     | sync        | read input registers                  |                                    |                                 | MB-040         |
| 7   |     `ModbusRegisterMap`     | `ModbusSlave`           | return      | register values                       |                                    |                                 | SA-150         |
...
**Use cases:** UC-18 (initiate update), UC-20 (firmware delivery).
...
| 8   | `UpdateService`       | `ResetDriver`       | sync   | trigger MCU reset                         |                  | DM-070               |
...
| REQ-DM-040  | Self-check after restart (sensor init, comms links)                        | IMPLIES MESSAGE (probes)                       |
| REQ-NF-203  | Recover from MCU reset within 5 s                                          | IMPLIES TIMING CONSTRAINT                      |

### 13.2 Step 2 — Participants

`Remote Operator`, `AWS IoT Core`, `MqttClient`, `CloudPublisher`, `LifecycleController`, `ResetDriver`, `Bootloader`, `SensorService`, `ModbusPoller`.
...
| —   |                       |                   |        |                                           | `[operator confirmed]`     |                              |                |
| 12  |   `LifecycleController` | `LifecycleController` | self | set pending_restart_ack flag           |                            |                              | DM-030         |
...
| —   |                       |                   |        |                                           | **ref** SD-00b (cold boot) |                              | NF-203         |
| 17  |   `LifecycleController` | `SensorService` | sync   | probe sensors                             |                            |                              | DM-040         |
| 18  |   `SensorService`     | `LifecycleController` | return | sensor status                           |                            |                              | DM-040         |
| 19  |   `LifecycleController` | `ModbusPoller`  | sync   | probe Modbus link                         |                            |                              | DM-040         |
| 20  |   `ModbusPoller`      | `LifecycleController` | return | link status                             |                            |                              | DM-040         |
| 21  |   `LifecycleController` | `CloudPublisher`| sync   | report restart success                    |                            |                              | DM-030         |
| 22  |   `CloudPublisher`    | `MqttClient`      | sync   | publish QoS 1 on result topic             |                            |                              | DM-030         |
| —   |                       |                   |        |                                           | **end alt**                |                              |                |

**UML note 1 (post-restart self-check):** *"Self-check after restart uses the same probe pattern as SD-06d (serial sync probes). The pending_restart_ack flag distinguishes a planned restart from a crash recovery so the gateway knows to publish a `restart success` message after the self-check passes."*

**UML note 2 (cold-boot reuse):** *"After the MCU reset (message 13) the gateway follows the standard SD-00b cold-boot path. Self-check probes (17–20) and result publication (21–22) happen *after* SD-00b's link-establishment. Post-boot resumption logic detects the `pending_restart_ack` flag and routes to the success-report path."*
...
| SD08-D2 | Self-check shares probe logic with SD-06d | Reuses serial-sync probe pattern; consistent across restart types |
| SD08-D3 | `pending_restart_ack` flag persists across reboot to enable success-reporting | RAM is lost at reset; flag in QSPI tells post-boot logic to publish ack |
...
`(NTP timer)`, `TimeService`, `NtpClient`, `WifiDriver`, NTP server (`«cloud»` for diagram purposes), `RtcDriver`, `ModbusMaster`, `ModbusUartDriver` (GW), `ModbusUartDriver` (FD), `ModbusSlave`, `ModbusRegisterMap` (FD), `Logger`, `CloudPublisher`.
...
| 10  |   `TimeService`       | `ModbusMaster`      | sync        | write time to FD (FC 06/16, holding reg)  |                            | TS-030, MB-040 |
...
| 14  |   `ModbusSlave`       | `ModbusRegisterMap` (FD) | sync   | write holding register (time)             |                            | MB-040         |
...
| D14 | Master-slave link establishment via per-slave probe with profile-bound device-ID validation; fall-through to Running on failure (Option C) | Industry-standard "deny by default"; supports REQ-NF-203 5 s boot budget                       |
