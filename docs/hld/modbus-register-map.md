...
| 0x0004 | FW_VERSION_MAJOR | uint16 | — | — | 0..255 | 0 | `firmware_version.h` (`FW_VERSION_MAJOR`) | `ModbusPoller` | Firmware major version (semver MAJOR). Generated by `scripts/gen-version.py` from `git describe` *(REQ-MB-111)*. |
| 0x0005 | FW_VERSION_MINOR | uint16 | — | — | 0..255 | 0 | `firmware_version.h` (`FW_VERSION_MINOR`) | `ModbusPoller` | Firmware minor version (semver MINOR) *(REQ-MB-111)*. |
| 0x0006 | FW_VERSION_PATCH | uint16 | — | — | 0..255 | 0 | `firmware_version.h` (`FW_VERSION_PATCH`) | `ModbusPoller` | Firmware patch version (semver PATCH) *(REQ-MB-111)*. |
