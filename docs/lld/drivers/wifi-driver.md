...
| WIFI-T22 | `wifi_recv` sets the module-side read timeout (WIFI-O16) | Sends `R2=<timeout_ms>` between `R1=` and `R0`; payload still returned correctly |
| WIFI-T23 | `wifi_close_socket` frees the local slot even when the P6=0 stop-client fails (WIFI-O17) | Returns `WIFI_ERR_SOCKET`, but the slot is reusable — a following `wifi_open_socket` succeeds |
...
| WIFI-O16 | Phase 4 hardware bring-up of the WifiTask non-blocking recv (TC-HW-CP-004, 2026-09-13): `wifi_recv()` never set IWIN's `R2` (read transport timeout), so an empty `R0` was answered only after the module's own power-on default hold-off — the uncontrollable "multi-second module-side wait" WIFI-O11 observed and worked around with a 5 s floor on the DRDY wait. Any caller asking for a short `timeout_ms` still paid ~5 s per empty read, which is what made WifiTask's background poll block its own task (and therefore the owner's queued `wifitask_send()`s) for 5 s per attempt. | **Resolved** | `wifi_recv()` now issues `R2=<timeout_ms>` right before `R0` (ST's reference `es_wifi.c` `ES_WIFI_ReceiveData` does the same), and `R0`'s DRDY wait is `timeout_ms + WIFI_RESP_TIMEOUT_MS` (one AT turnaround of margin) instead of the floor. Confirmed on hardware: an empty 200 ms poll returns in ~200 ms. The documented "blocks until data is available or timeout expires" contract is now enforced by the module. Covered by WIFI-T22; the three existing recv tests gained one scripted `OK` for the extra command. |
| WIFI-O17 | Phase 4 hardware bring-up of reconnect (2026-09-13): after a broker/peer TCP reset, every reconnect failed at `wifitask_open_socket` with `WIFI_ERR_SOCKET`. `wifi_close_socket()` sent P0 then P6=0 and returned early on the P6=0 error *without* clearing `socket_open[socket]` — but P6=0 routinely fails against a peer that has already RST the session, so the local slot leaked and the module socket was left half-torn-down, wedging subsequent opens. | **Resolved** | `wifi_close_socket()` now clears `socket_open[socket]` up front, unconditionally, before issuing the teardown AT commands (it still reports their error for diagnostics): the socket is gone as far as the driver is concerned once close is requested, so the slot is always reusable. Confirmed on hardware — reconnect after a broker restart now completes in ~12 s and publishes resume. Covered by WIFI-T23. Together with the WifiTask recv-slot reset (wifi-task.md, same date) this closes the reconnect path. |
