...
| CP-T20 | `MQTT_CLIENT_WIFI_RECV_READY_BIT` wake (WIFITASK-O1 Phase 3, CP-D11) | Notification with only this bit set (not `CP_NOTIFY_STATS_TICK`) still calls `prv_poll_stats` |
| CP-T21 | Wake bit cleared, no busy-loop (CP-D11) | After one step consuming the bit, a second step with no re-arm sees no pending notification |
| CP-T22 | Alarm drained before telemetry/health in one wake (CP-D12) | A notification with both `CP_NOTIFY_ALARM_PENDING` and `CP_NOTIFY_TELEMETRY_TICK` set publishes the alarm before the telemetry frame |
| CP-T25 | Reconnect backoff is exponential and resets (CP-D13) | Successive terminal failures back off 2 s then 4 s (doubling); a successful connect resets the curve so the next drop retries immediately then backs off from 2 s again |
| TC-HW-CP-010 | Data-integrity (hardware, WIFITASK-O1 Phase 4): 20 patterned inbound payloads 32-4038 B straddling the module's 1460 B R0 chunk | Every byte and the sequence verified by bringup_msg_cb(); PASS on hardware 2026-09-13 (33 948 B, 0 corrupt, 0 out-of-order) |
| TC-HW-CP-011 | Alarm-latency (hardware, REQ-NF-113): synthetic alarm every 700 ms, detection->outcome timed against 500 ms | See CP-O6 — steady-state median 347 ms but ~5-9% over 500 ms, and a ~15 s stall on the first failed publish after a broker drop; recorded, **budget not met** |
| TC-HW-CP-012 | Wallclock-parity (hardware): initial connect + reconnect wall-clock vs the pre-Phase-2 ~30 s baseline | Initial TCP+TLS+MQTT connect ~4.7 s on hardware 2026-09-13 (PASS, no regression); reconnect time is informational (dominated by broker-down dwell + reconnect backoff, WIFITASK-O2) |
...
| CP-O6 | REQ-NF-113 (alarm detection->publish within 500 ms) is **not reliably met** with the current synchronous QoS1 alarm path. Measured on hardware (TC-HW-CP-011, 2026-09-13): a single QoS1 publish is ~347 ms median (one ISM43362 PUBLISH+PUBACK round trip, ~70% of the budget), so steady-state ~5-9% of alarms exceed 500 ms whenever one lands while CloudPublisherTask is mid-publish (telemetry/health/inbound), which the single-threaded task cannot preempt. Worse, the **first** alarm after a broker drop blocks the task ~15 s on `MQTT_PUBACK_TIMEOUT_MS` before the connection is marked down; alarms pile up until then (though once down, each buffers to SAF in <1 ms, safely). CP-D12 reordering helps the co-wake case only. | **Largely resolved (MQTT-D13)** | Made `mqtt_client_publish()` non-blocking (option (b)): it transmits and returns, the QoS 1 PUBACK is reaped in `mqtt_client_process()`, and the send BIO fast-fails a dead socket instead of looping to `MQTT_SEND_TIMEOUT_MS`. Measured on hardware: the **~15 s broker-drop stall is gone** (a failed send returns `PUBLISH_FAIL` fast -> SAF; once disconnected, alarms buffer in <1 ms) and the steady-state **tail dropped** (max ~766 ms blocking -> ~584 ms non-blocking). Honestly **not** fully closed: a single alarm's floor is the ISM43362 send round trip itself (~350 ms median, ~70% of the 500 ms budget — WIFI-O11 module latency, not the PUBACK wait, which was never the median cost against a fast broker), so a co-in-flight publish on the single CloudPublisherTask can still spike an alarm past 500 ms occasionally. A hard 500 ms guarantee would need faster module I/O or a dedicated alarm socket/task (out of scope).  **Hardware-confirmed 2026-09-13 (broker-down scenario):** with the broker restarted mid-flight, **240/240 alarms handled, max 125 ms, 0 over budget, no stall** (the old build stalled ~15 s on the first failed publish) — every alarm buffered to SAF within budget while disconnected. One subtlety the hardware surfaced: making the send BIO fatal-on-error must be scoped to an *established* connection (`inst->connected`); during a (re)handshake the ISM43362 can briefly return `WIFI_ERR_SOCKET` on a just-opened socket, so `prv_mbedtls_net_send()` returns `WANT_WRITE` there to let mbedTLS retry across `connect_step()` ticks — otherwise every reconnect aborts on that transient. **Reconnect now confirmed 2026-09-13:** after fixing two further hardware-only bugs — the WifiTask recv slot not resetting across a socket reopen (stale POLL_ERROR -> handshake -0x6800; wifi-task.md) and `wifi_close_socket()` leaking the local slot when the stop-client command fails on a dead peer (WIFI-O17) — a broker restart mid-flight now recovers cleanly: disconnect detected, alarms buffered within budget throughout (0 over 500 ms across the drop), **MQTT (re)connected in ~12 s**, broker receiving publishes again. (Earlier note, now superseded — the first attempt was **not re-confirmed this run: TCP reconnect recovery after the peer reset** — the test module had degraded after a long session of re-associations (it also stopped re-joining Wi-Fi, recovering only on a USB power-cycle, not an RST-pin reset), so its socket would not recover after the broker's RST; the reconnect code path is unchanged from the earlier run that did reconnect (~54 s). Reconnect robustness against a peer reset (module socket recovery / settle-or-reset policy) is tracked under WIFITASK-O2. Original options considered were: (a) publish alarms QoS0 (fire-and-forget, no PUBACK wait — trades the at-least-once guarantee for latency; SAF still covers the disconnected case); or (b) make `mqtt_client_publish()` non-blocking for QoS1 (return after `MQTT_Publish()` transmits, reap the PUBACK in `mqtt_client_process()`), which also removes the ~15 s broker-death stall; or (c) confirm REQ-NF-113 means "PUBLISH transmitted" not "PUBACK received", under which the transmitted-latency already fits. Residual: a PUBACK lost on a still-live connection is untracked (inherent QoS 1/clean-session, bounded by keep-alive). Ties into MQTT-O9 (the connect/publish paths that still block a whole tick) and WIFITASK-O2 (reconnect policy). |
...
| CP-D10 | `prv_maybe_reconnect()` calls the new ticked `mqtt_client_connect_step()` (mqtt-client.md MQTT-D8) instead of the blocking `mqtt_client_connect()` | Hardware testing of CP-D9 found `mqtt_client_connect()` could freeze `CloudPublisherTask` — and with it telemetry, health, alarm, and command handling, all driven from the same task — for ~19 s in a single call on a failed reconnect (TLS handshake timeout), risking REQ-NF-113 (≤500 ms alarm-to-publish-queue latency) if an alarm fires mid-reconnect. `connect_step()` advances at most one bounded phase per call, with progress persisted on the MqttClient handle rather than here, so `prv_maybe_reconnect()` needs no new state of its own beyond the existing `reconnect_countdown_s` backoff — it just keeps calling `connect_step()` every stats tick while disconnected and not backing off. `MQTT_CLIENT_ERR_IN_PROGRESS` is treated as "retry next tick, no backoff" (CP-T19), distinct from `MQTT_CLIENT_ERR_OK` (connected) and a terminal failure (backoff armed, unchanged from CP-D9). Deliberately scoped to MqttClient only — WifiDriver's blocking transport was considered and declined for this change (see mqtt-client.md MQTT-O9): it's an intentional, already-tagged, hardware-validated architectural boundary with a documented regression history in that exact code path, and reworking it is its own separately-scoped decision. |
| CP-D11 | `CP_NOTIFY_ALL_BITS` extended to include `MQTT_CLIENT_WIFI_RECV_READY_BIT` (mqtt-client.md MQTT-D9, `wifi-task.md` WIFITASK-O1 Phase 3), and `prv_task_step()`'s guard around `prv_poll_stats()` changed from `CP_NOTIFY_STATS_TICK` alone to `CP_NOTIFY_STATS_TICK \| MQTT_CLIENT_WIFI_RECV_READY_BIT` | CP-D10's `connect_step()` ticking is only as responsive as whatever wakes `prv_task_step()` — without this, a TLS handshake that just received real data via `wifitask_try_recv()` (MqttClient) would still sit idle until the next 1 Hz stats tick, adding up to ~1 s of pure dead time per round-trip on top of WIFI-O11's own irreducible module latency, a regression relative to the pre-Phase-2 blocking behaviour it was meant to improve on. The bit must also be in `CP_NOTIFY_ALL_BITS` for a second reason beyond waking the task: that same constant is passed as `xTaskNotifyWait()`'s clear-on-exit mask, and any bit excluded from it is never cleared from the notification word — `eSetBits` would still wake the task on it (any bit change unblocks `xTaskNotifyWait` regardless of mask), but it would then stay set forever, making every subsequent wait call return immediately: a busy-loop, not a missed wake (CP-T21 regression-tests specifically this, not just the wake itself, which CP-T20 covers). The bit itself is owned and defined by MqttClient (`MQTT_CLIENT_WIFI_RECV_READY_BIT`, `mqtt_client.h`), not CloudPublisher, since MqttClient is the module that actually calls `wifitask_try_recv()` and sets it — CloudPublisher only consumes it, and already includes `mqtt_client.h`. |
| CP-D12 | `prv_task_step()` drains the alarm and command queues **before** the periodic telemetry/health publishes | Hardware finding (TC-HW-CP-011): all four paths call `prv_enqueue_or_publish()`, which for QoS1 blocks the single CloudPublisherTask ~350 ms on the PUBACK. Handling a telemetry publish first pushes a co-woken alarm past REQ-NF-113's 500 ms budget. Reordering fixes the co-wake case (it cannot preempt a publish already in flight — that residual is CP-O6). |
| CP-D13 | Reconnect uses an exponential backoff after a terminal `connect_step()` failure: `CP_RECONNECT_BACKOFF_MIN_S` (2 s), doubling per consecutive failure up to `CP_RECONNECT_BACKOFF_MAX_S` (60 s), reset to MIN on a successful connect (WIFITASK-O2). | The prior policy was a single fixed 30 s retry. A backoff bounds how hard a persistently-down broker is re-hammered and, importantly, spaces out the WiFi (re)associations that stress the AP — the same association churn that made the bring-up bench flaky. MIN is >= 2 s because the retry countdown ticks at the 1 Hz stats rate, so a 1 s floor would let a retry slip through on the next tick. `IN_PROGRESS` still retries every tick (not a failure); the backoff only arms on a terminal error. |
| CP-D14 | No reconnect give-up: `prv_maybe_reconnect()` retries **indefinitely** with the CP-D13 capped backoff — there is no maximum attempt count after which it stops. | Pins the last part of WIFITASK-O2 (attempt-count policy). The Gateway is mains-powered and always-on; a transient outage of minutes or hours (broker maintenance, AP reboot, ISP blip) must self-heal without intervention, so permanently going dark after N attempts would be a defect, not a safeguard. Unbounded retry is safe because the two things it could exhaust are both already bounded: per-tick blocking is capped (CP-D10/MQTT-O9) so the task stays responsive, and offline payloads accumulate in the bounded StoreAndForward buffer, which drops oldest-first when full (CP-T15, SAF_ERR_FULL) rather than growing without limit. Attempts are observable — `mqtt_stats.connect_attempts` climbs while `connect_ok`/`reconnect_count` stay flat during an outage — and reach the health report via `prv_poll_stats()` -> `health_report_update_mqtt()`, so a sustained disconnection is visible to operators without a give-up. Re-establishing the *WiFi association* (as opposed to the socket) on a genuine AP drop is WifiTask's own liveness job (WIFI_LIVENESS_CHECK_PERIOD_MS), layered below this and not CloudPublisher's concern — CloudPublisher holds no WiFi handle by design. |
