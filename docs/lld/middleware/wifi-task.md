...
| WIFITASK-O2 | WIFI-O15's reconnect **policy** (attempt count, backoff curve, and what happens to any socket/MQTT session open at the time of an AP drop) is not pinned — only the structural owner and tick (§5.3) are. | **Backoff pinned (CP-D13); socket handling resolved (WIFI-O17)** | The reconnect **backoff** is now concrete: CloudPublisher (the reconnect owner, CP-D9/D10) applies an exponential curve — 2 s doubling to a 60 s cap, reset on success (cloud-publisher-lld.md CP-D13, CP-T25). The **socket handling on drop** is settled too: `wifi_close_socket()` frees its local slot unconditionally (wifi-driver.md WIFI-O17) and WifiTask resets its recv slot on socket open/close (WIFITASK-T26), so a dropped session's socket and recv state don't wedge the next connect — hardware-confirmed reconnect in ~12 s. Remaining sub-question (attempt **count** / give-up policy) is deferred until a second real caller exists, as originally noted — the current policy retries indefinitely with the capped backoff, which is correct for a mains-powered always-on gateway. |
