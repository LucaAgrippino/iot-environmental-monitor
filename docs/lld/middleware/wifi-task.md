...
**Update (WIFITASK-O1 Phases 1-3):** the "concrete, narrower path forward"
...
MqttClient's production recv path (`prv_mbedtls_net_recv()`) now calls
`wifitask_try_recv()` (Phase 2), and CloudPublisher re-drives the connect
sequence promptly on the resulting wake bit rather than waiting for its
1 Hz stats tick (Phase 3) — MQTT-O7 (mqtt-client.md) is resolved as of
this update; see §10 below for what remains (Phase 4 hardware validation,
Phase 5 deferred unification).
...
**WIFITASK-O1 Phase 4 — hardware findings fixed (§10, WIFITASK-O1):**

| ID | Scenario | Expected |
|---|---|---|
| WIFITASK-T21 | `wifitask_try_recv()` while the slot is IDLE | Besides the arm, a NULL "kick" is sent into `request_queue` so WifiTask leaves its 30 s liveness wait immediately |
| WIFITASK-T22 | A step receives that NULL kick | Wake only: no dispatch, no reply notify, no `wifi_recv()` |
| WIFITASK-T23 | An 8-byte DONE payload picked up as 5 then 16 bytes | Delivers "ABCDE" then "FGH" (consecutive, never re-delivered), then reads as `PENDING` |
| WIFITASK-T24 | An arm arrives while an OK payload is partly unconsumed | Dropped — no new `wifi_recv()` attempt until the owner drains it; the next arm after that re-arms |
| WIFITASK-T25 | Background attempt with no data | `wifi_recv()` called with the short `WIFITASK_RECV_POLL_TIMEOUT_MS`, and the owner is **not** woken for an empty result |
...
|---|---|---|---|
| WIFITASK-O1 | MQTT-O7 (CloudPublisherTask's 5 s idle-recv stall) is relocated, not fixed. `wifitask_recv()` still blocks the caller for up to `WIFI_RESP_TIMEOUT_MS` (5 s), same as `wifi_recv()` does today — WifiTask just changes *where* that block executes. | **Phases 1-4 shipped and hardware-validated; formal Phase 4 test cases and Phase 5 open** | Phase 1 (WifiTask layer only, §4.3a): `wifitask_try_recv()` — genuinely non-blocking, backed by a per-slot background poll inside WifiTask, existing alongside the unchanged `wifitask_recv()`. Covered by WIFITASK-T17..T20. Phase 2: `mqtt_client.c`'s `prv_mbedtls_net_recv()` (the *only* real production call site) rewired to call `wifitask_try_recv()`, mapping `READY`→byte count, `PENDING`/`NONE`→`MBEDTLS_ERR_SSL_WANT_READ`, `ERROR`→`MBEDTLS_ERR_SSL_TIMEOUT`; no new state machine needed since `mbedtls_ssl_handshake()` already tolerates `WANT_READ` (that's what already powers `mqtt_client_connect_step()`'s ticking, MQTT-D8). `prv_transport_recv()`'s `#ifdef TEST` body deliberately left on the old blocking `wifitask_recv()` — it bypasses TLS entirely to let coreMQTT parse synthetic cleartext frames in host tests and has no production role. Phase 3: the wake bit is `MQTT_CLIENT_WIFI_RECV_READY_BIT` (`mqtt_client.h`, bit 5) — owned by MqttClient, not CloudPublisher (`CP_NOTIFY_WIFI_RECV_READY` as originally sketched would have been backwards: MqttClient is the one that sets it via `wifitask_try_recv()`'s `ready_notify_bit`, and CloudPublisher already includes `mqtt_client.h`). `CP_NOTIFY_ALL_BITS` extended to include it (both as the wake condition *and*, critically, in `xTaskNotifyWait()`'s clear-on-exit mask — omitting it there would still wake the task via `eSetBits` but never clear the bit, a busy-loop). `prv_task_step()`'s guard around `prv_poll_stats()`/`prv_maybe_reconnect()` now fires on `CP_NOTIFY_STATS_TICK | MQTT_CLIENT_WIFI_RECV_READY_BIT`. Covered by CP-T20 (wake triggers a poll) and CP-T21 (bit is actually cleared, not just wakes once). **Remaining:** Phase 4 — hardware validation: TC-HW-WIFITASK-alarm-latency (REQ-NF-113 stays ≤500 ms during an in-flight reconnect), TC-HW-WIFITASK-wallclock-parity (**no regression** against the current ~30 s baseline — not claiming improvement, see below), TC-HW-WIFITASK-data-integrity. Phase 5 (deferred, not currently planned) — internal unification of `wifitask_recv()` on top of `wifitask_try_recv()`; a complementary, separately-scoped DATARDY-notification-based wait inside WifiDriver itself (frees the CPU during WifiTask's own ~5 s attempt, for tasks *other* than the caller — a different axis from this item, not a superset of it; not bundled here, same precedent as CP-D10/MQTT-O9 declining a WifiDriver touch inside an adjacent fix). **Phase 4 result (2026-09-13, first run of Phases 1-3 on hardware, Linux bench, home router + Dockerised Mosquitto):** TC-HW-CP-004 failed outright, three times over, each layer hiding the next — none of it visible to host tests because none of it is logic, it is timing and byte-stream semantics. (1) *The wake:* with the slot IDLE, `prv_wifitask_step()` parks in `xQueueReceive(request_queue)` for `WIFI_LIVENESS_CHECK_PERIOD_MS` (30 s) and only polls the arm queue at the top of a step, so the first `wifitask_try_recv()` after any idle period waited up to 30 s — exactly MqttClient's handshake deadline; a packet capture showed the module ACKing the whole server flight at +7 ms and the firmware never reading it. Fixed: `try_recv()` from IDLE also sends a NULL pointer "kick" into `request_queue` (value-typed, nothing to dereference — not WIFITASK-O6's class; no heap/queue sets available for the textbook answer), which the step treats as wake-only. (2) *Stream semantics:* pickup copied `min(scratch_len, buf_len)` from offset 0 and left the slot DONE, so mbedTLS's 5-byte-header-then-body reads got the header twice and lost the rest, and the re-arm sent by the first pickup let a new attempt overwrite `scratch` under it. Fixed: `scratch_off` consumption cursor, pickup/publish in a critical section, re-arm refused while an OK payload is unconsumed (dropped arms are harmless — the owner re-sends one per call). (3) *Poll cadence:* each background attempt used the 5 s `WIFITASK_WIFI_RESP_TIMEOUT_MS`, blocking WifiTask — and therefore the same owner's queued `wifitask_send()`s — for 5 s per empty poll; the handshake's client flight (4 sends) drifted to 19 s+ between records. Root cause was one layer down (WIFI-O16: `R2` never set, so the module chose the hold-off); with that fixed, attempts use `WIFITASK_RECV_POLL_TIMEOUT_MS` (200 ms) and an empty attempt no longer wakes the owner (it re-armed on every wake and the pair spun at 4 Hz, visible as CloudPublisher's 1 Hz stats tick logging 4×/s). Separately, the remaining wall-clock was crypto, not I/O: RSA-2048 CertificateVerify + P-256 ECDHE at `-O0` alone exceeded 30 s (Mosquitto 2.x also drops un-CONNECTed clients at 30 s), fixed by compiling `mbedtls-library` at `-O2` in the Debug config (`.cproject` folder override) with `MBEDTLS_HAVE_ASM` + `MBEDTLS_ECP_NIST_OPTIM` — see mqtt-client.md MQTT-O10. **Outcome:** TC-HW-CP-004 connects in **~4 s** (was ~30 s on the pre-Phase-2 blocking path, never on Phases 1-3); CP-005 SUBACK, CP-006 create, CP-007/008 telemetry + health observed at the broker, CP-009 stats at 1 Hz, and two externally published config commands delivered through `try_recv()` in <1 s. Covered by WIFITASK-T21..T25 and WIFI-T22. The earlier claim that the ~30 s floor was module-side latency was wrong — it was mostly `-O0` crypto plus the unset `R2`. Phase 4's three named cases are now written and run on hardware (2026-09-13), in main_test_cloud_publisher.c as TC-HW-CP-010/011/012 with scripts/bringup-data-integrity.py: **data-integrity PASS** (33 948 B across chunk-straddling payloads, zero corruption/reordering — the strongest proof the stream-cursor fix is correct end to end), **wallclock-parity PASS** (initial connect ~4.7 s vs the ~30 s baseline, no regression). **alarm-latency did not pass** and surfaced a real requirement gap that is not a WifiTask defect but a CloudPublisher/MqttClient one — the synchronous QoS1 publish path, tracked as cloud-publisher-lld.md CP-O6. Reconnect after a broker/peer reset is now hardware-confirmed (2026-09-13) once two further bugs were fixed: this recv-slot reset (prv_recv_slot_reset on socket open/close, WIFITASK-T26) and wifi-driver.md WIFI-O17 (close_socket slot leak) — a mid-flight broker restart recovers in ~12 s with alarms buffered within budget throughout. Phase 5 unchanged. |
| WIFITASK-O2 | WIFI-O15's reconnect **policy** (attempt count, backoff curve, and what happens to any socket/MQTT session open at the time of an AP drop) is not pinned — only the structural owner and tick (§5.3) are. | **Resolved** | All three parts now pinned. **Backoff curve:** CloudPublisher applies an exponential curve — 2 s doubling to a 60 s cap, reset on success (cloud-publisher-lld.md CP-D13, CP-T25). **Socket/session handling on drop:** `wifi_close_socket()` frees its local slot unconditionally (wifi-driver.md WIFI-O17, WIFI-T23) and WifiTask resets its recv slot on socket open/close (WIFITASK-T26), so a dropped session's socket and recv state don't wedge the next connect — hardware-confirmed reconnect in ~12 s. **Attempt count / give-up:** no give-up — indefinite retry with the capped backoff (CP-D14), correct for a mains-powered always-on gateway; safe because per-tick blocking is bounded and offline payloads land in the bounded, drop-oldest StoreAndForward buffer; attempts are observable via `mqtt_stats.connect_attempts`/`reconnect_count` in the health report. |
...
Two open items remain (WIFITASK-O4 liveness period, WIFITASK-O5 timeout
constants); WIFITASK-O1 (non-blocking recv, hardware-validated) and
WIFITASK-O2 (reconnect policy — backoff, socket handling, and give-up all
pinned) are now resolved. Neither remaining item blocks *this* document's
implementation-readiness; both are scoped follow-ups with a stated path
forward, consistent with how
