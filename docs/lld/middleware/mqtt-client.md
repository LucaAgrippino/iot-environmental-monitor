...
|---|---|---|---|
| MQTT-O1 | mbedTLS RAM: ~35–50 KB. Must verify against 128 KB SRAM budget at integration. | **Open** | Verify at integration. If insufficient, evaluate reduced cipher config or on-module TLS fallback. |
| MQTT-O2 | `MQTT_CONNECT_TIMEOUT_MS` = 10 000 ms (provisional). TLS handshake bottleneck ~2–5 s. | **Open** | Validate at integration. |
...
| MQTT-O4 | `MQTT_PUBACK_TIMEOUT_MS` = 5000 ms (provisional). | **Open** | Validate against observed AWS IoT Core RTT at integration. |
