...
- **Sensors:** On-board temperature, humidity, and pressure sensors
...
The field device reads on-board sensors (temperature, humidity, pressure) at a configurable polling interval. Sensor readings are processed and made available via Modbus registers for the gateway to read, and displayed on the local LCD.
